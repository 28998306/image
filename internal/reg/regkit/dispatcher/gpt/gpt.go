...
import (
	...
	"web2img/internal/reg/regkit/nameset"
...
const (
	...
	// 与官方 Codex CLI 的 OAuth 参数一致，产物可直接当 Codex token 使用。
	...
	// 注册流程跑完之后我们走 codex 的 /oauth/authorize 拿真正可用的 token。
...
func (d *Dispatcher) Run(ctx context.Context, svc *service.RegisterTaskService, task *model.RegisterTask) error {
	...
	//        调 /v1/models /v1/chat/completions 等 API。
	//      * refresh_token 也是 platform 的，跟 codex 隔离（同一账户可以共存）。
	//
	//    暂时不做 token-exchange 拿 sk- api_key（codex token 跟 platform 隔离）。
...
//
// 这是 token-exchange 换取 sk- api_key 的关键产物。
//
...
//   - audience=api.openai.com/v1 → 拿到的 access_token 可直接调 api.openai.com/v1/*。
//
// 与 codex 流程不同：
...
// platformLoginAndExchange —— platform OAuth 登录换 token：
//
...
func (d *Dispatcher) platformLoginAndExchange(ctx context.Context, bc *browser.Client, st *state,
	...
	// step 2（首选）：刚注册完，注册阶段的会话**已经是登录态**。此时对已登录会话再走
	// 一遍"登录"（authorize/continue + password/verify）会被 OpenAI 判为 invalid_state
	// （"sign-in session is no longer valid"）——这正是之前一直失败的根因。
	// 正确做法是直接 GET authorize 跟着 consent 链拿 ?code=，完全跳过密码登录。
	if code, _, ferr := codexFollowAndConsent(ctx, bc, st, authURL); ferr == nil && code != "" {
		svc.LogInfo(ctx, taskID, "platformLogin: 复用已登录会话直接拿到 ?code=，跳过密码登录")
		return doTokenExchangePlatform(ctx, bc, st, code)
	} else if ferr != nil && !errors.Is(ferr, errLoginPasswordRequired) && !errors.Is(ferr, errAddPhoneRequired) {
		svc.LogWarn(ctx, taskID, fmt.Sprintf("platformLogin: 已登录会话直连 consent 未拿到 code（%v），回退密码登录流程", ferr))
	} else {
		svc.LogInfo(ctx, taskID, "platformLogin: 会话需重新登录，进入密码登录流程")
	}

	// step 3（回退）：会话确实需要重新登录时才走密码流程。
	// authorize → continue(screen_hint=login) → password/verify。
	if err := primeAuthorize(ctx, bc, st, authURL); err != nil {
		return nil, fmt.Errorf("platform authorize: %w", err)
	}
	if err := postLoginContinue(ctx, bc, st, st.email); err != nil {
		return nil, fmt.Errorf("platform authorize/continue(login): %w", err)
	}
	svc.LogInfo(ctx, taskID, "platformLogin: authorize + continue(login) 完成，会话已进入 login_password")
	...
	for attempt := 1; attempt <= maxStateRetry; attempt++ {
		...
		svc.LogWarn(ctx, taskID, fmt.Sprintf("platformLogin: password/verify HTTP 409 invalid_state（第 %d 次），重建全新 OAuth session 重试", attempt))
		time.Sleep(800 * time.Millisecond)
		// invalid_state 的根因：注册阶段遗留的 OAuth session 状态（stale oai-state /
		// login_session cookie + 注册期 sentinel proof）跟新 PKCE/state 对不上。
		// 仅清 cookie 不够——必须像 freshCodexLogin 那样做"全套重置"：
		//   1. 清 OAuth session cookies（保留 oai-did）
		//   2. 用同一 oai-did 重建 sentinel（拿到与新会话匹配的 proof requirements）
		//   3. 重新 set oai-did，确保设备一致
		//   4. 新 PKCE/state/nonce 后重新 GET authorize
		...
		// 重试同样要重新走 continue(login)，把会话推进到 login_password。
		if perr := postLoginContinue(ctx, bc, st, st.email); perr != nil {
			return nil, fmt.Errorf("retry authorize/continue(login): %w", perr)
		}
	}
