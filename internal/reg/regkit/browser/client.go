...
type Options struct {
	...
	// ForceHTTP1 强制只用 HTTP/1.1（ALPN 只声明 http/1.1）。
	// 用于 chatgpt.com 网页接口（/、/backend-api/sentinel/*、/backend-api/f/*）：
	// 这些路由有 Cloudflare bot 风控，会比对 HTTP/2 层指纹（SETTINGS/伪头顺序），
	// Go 的 http2.Transport 指纹非 Chrome 会被直接 403。强制 http/1.1 可规避。
	ForceHTTP1 bool
}
...
func New(opts Options) (*Client, error) {
	...
	transport, err := newTransport(opts.ProxyURL, timeout, opts.ForceHTTP1)
	if err != nil {
		return nil, err
	}
	prof := RandomProfile()
...
func newTransport(proxyURL string, timeout time.Duration, forceHTTP1 bool) (http.RoundTripper, error) {
	t := &uTLSTransport{timeout: timeout, forceHTTP1: forceHTTP1}
	if proxyURL != "" {
...
func (t *uTLSTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if t.forceHTTP1 {
		// 只声明 http/1.1，规避 Go http2 层指纹被 Cloudflare 风控识别。
	...
	if t.forceHTTP1 {
		// 预设 ClientHello 的 ALPN 扩展默认含 h2，需显式改成只剩 http/1.1。
	...
	hsCtx, hsCancel := context.WithTimeout(req.Context(), t.connectTimeout())
	...
	if !t.forceHTTP1 && tlsConn.ConnectionState().NegotiatedProtocol == "h2" {
...
// connectTimeout 建连/握手阶段的上限，避免 chatgpt.com 不可达时一直等到整体超时（最长几分钟）。
func (t *uTLSTransport) connectTimeout() time.Duration {
	if t.timeout > 0 && t.timeout < 45*time.Second {
		return t.timeout
	}
...
func (t *uTLSTransport) dialTarget(ctx context.Context, target *url.URL) (net.Conn, error) {
	...
	if t.proxyURL == nil {
		return (&net.Dialer{Timeout: t.connectTimeout(), KeepAlive: 30 * time.Second}).DialContext(ctx, "tcp", addr)
	}
...
func (t *uTLSTransport) dialHTTPProxy(ctx context.Context, targetAddr string) (net.Conn, error) {
	...
	conn, err := (&net.Dialer{Timeout: t.connectTimeout(), KeepAlive: 30 * time.Second}).DialContext(ctx, "tcp", proxyAddr)
	if err != nil {
