...
// FetchRecent 打开指定邮箱并拉取最近邮件（只读，不改 DB 状态）。
func (m *Manager) FetchRecent(ctx context.Context, mailID uint64, cfg BackendConfig, limit int) ([]MailMessage, error) {
	row, err := m.pool.GetByID(ctx, mailID)
	if err != nil {
		return nil, err
	}
	box, err := m.openWithRow(ctx, row, cfg)
	...
	defer box.Close()
	lister, ok := box.(MessageLister)
	if !ok {
		return nil, fmt.Errorf("邮箱后端 %q 暂不支持收件预览", row.Mode)
	}
	return lister.ListRecent(ctx, limit)
}
